...
/// For example:
///
///     GSTimer(interval: delay, handler: { (timer) in
///          self.containers[type.hudIdentify()]?.0.hide(animated: true, completion: completion)
///     })
public class GSTimer {
    
    public typealias GSTimerHandler = (GSTimer) -> Void
    
    private let internalTimer: DispatchSourceTimer
    public private(set) var isRunning = false
    private let repeats: Bool
    private var handler: GSTimerHandler
    
    public init(interval: DispatchTimeInterval, repeats: Bool = false, queue: DispatchQueue = .main, handler: @escaping GSTimerHandler) {
    ...
    public func rescheduleHandler(handler: @escaping GSTimerHandler) {
        self.handler = handler
        internalTimer.setEventHandler { [weak self] in
            if let strongSelf = self {
                handler(strongSelf)
...
extension GSTimer {
    public static func repeaticTimer(interval: DispatchTimeInterval, isWallTime: Bool = false, queue: DispatchQueue = .main, handlder: @escaping GSTimerHandler) -> GSTimer {
        return GSTimer(interval: interval, repeats: true, queue: queue, handler: handlder)
    }
}
...
///     })
public class GSCountDownTimer {
    
    private let internalTimer: GSTimer
    private var leftTimes: Int
    private var originalTimes: Int
    private var handler: (GSCountDownTimer, _ surplus: Int) -> Void
    
    public var isRunning: Bool {
        return internalTimer.isRunning
    }
    
    public init(interval: DispatchTimeInterval, times: Int, isWallTime: Bool = false, queue: DispatchQueue = .main, handler: @escaping ((GSCountDownTimer, _ surplus: Int) -> Void)) {
        self.leftTimes = times
        self.originalTimes = times
        self.handler = handler
        self.internalTimer = GSTimer.init(interval: interval, repeats: true, queue: queue, handler: { _ in})
    ...
    @discardableResult
    public func reSetConfig(newTimes: Int? = nil, interval: DispatchTimeInterval, handler: @escaping ((GSCountDownTimer, _ surplus: Int) -> Void)) -> Self {
        suspend()
